---
name: openspec-github-orchestrator
description: >-
...
Act as a resumable state-machine coordinator between OpenSpec and GitHub.
...
Use `openspec-propose` for normal planning and `openspec-update-change` when an
existing plan needs revision. If artifacts are missing, use the OpenSpec CLI's
`status` and `instructions` commands to identify and create the next artifact.
...
continue to implementation after the planning artifacts are coherent.
...
Ensure a feature branch exists, then use `openspec-apply-change`.
...
`openspec instructions` to identify and create the next required artifact.
...
Perform verification against the proposal/spec/design/tasks and actual
implementation evidence. Report OpenSpec verification as `ready`, `blocked`,
or `warnings`.
