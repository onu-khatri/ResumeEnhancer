...
- Development handoff: `$openspec-orchestrator` coordinates approved development work and is mandatory in `$issues-kickoff` before implementation ownership is handed off. `issues-kickoff` creates the canonical branch/worktree after approval; implementation agents are reached only through the OpenSpec coordinator.
