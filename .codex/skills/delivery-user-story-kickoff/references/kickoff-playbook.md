...
Operational reference for running `delivery-user-story-kickoff`. Use these templates and conventions to keep story-to-GitHub handoff consistent and traceable. Implementation execution belongs to `delivery-issues-kickoff`.
...
`delivery-user-story-kickoff` may transition `Ready_To_Implement` to `Move_To_GitHub_Issue` only through a successful `$delivery-github-issue` handoff. `approved` is not a lifecycle value. `delivery-issues-kickoff` owns later status transitions and branch/worktree fields, using the canonical `openspec/...` branch convention.
...
After handoff, stop. Use `delivery-issues-kickoff` for implementation workstream reporting and verification only after the user explicitly invokes it.
