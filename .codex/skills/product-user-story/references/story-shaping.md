...
Use this reference when selecting a story type, breaking down a capability, or turning requirements into testable acceptance criteria. It supports authoring only; `$delivery-user-story-kickoff` owns GitHub-handoff sequencing and `$delivery-issues-kickoff` owns implementation sequencing, worktrees, and agent assignment.
