...
export function useResumeProgress(
        ...
        return [
            { complete: currentValues.title.trim().length > 0, label: 'Title' },
            {
                complete:
                    ...
                    currentValues.personalInformation.phoneNumber.trim()
                        .length > 0,
                label: 'Contact',
            },
            {
                complete: currentValues.summary.trim().length >= 80,
                label: 'Summary',
            },
            {
                complete: currentValues.skills.some((item) =>
                    item.skillName.trim(),
                ),
                label: 'Skills',
            },
            ...
            {
                complete: currentValues.projects.some((item) =>
    ...
    const completionPercent = Math.round(
        (progressItems.filter((item) => item.complete).length /
