    ...
    >
      {(_field, index) => (
        <div className="grid gap-4 md:grid-cols-3">
          <InputField
            error={form.formState.errors.skills?.[index]?.skillName?.message}
            label="Skill"
            placeholder="React"
            {...form.register(`skills.${index}.skillName`)}
          />
          <InputField
            error={form.formState.errors.skills?.[index]?.proficiencyLevel?.message}
            label="Proficiency"
            placeholder="Advanced"
            {...form.register(`skills.${index}.proficiencyLevel`)}
          />
          <InputField
            error={form.formState.errors.skills?.[index]?.yearsOfExperience?.message}
          ...
          <TextareaField
            ...
            error={form.formState.errors.skills?.[index]?.description?.message}
            label="Context"
