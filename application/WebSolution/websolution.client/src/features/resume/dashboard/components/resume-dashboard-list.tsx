        ...
        >
          <div className="flex flex-col gap-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-slate-700 dark:text-slate-300">
                  {resume.summary?.trim() || 'No summary added yet.'}
                </p>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <MetaTile label="Updated" value={formatDate(resume.app_UpdateDate ?? resume.app_CreateDate)} />
              <MetaTile
                label="Sections"
            ...
            <div className="flex flex-wrap gap-3">
              <Button onClick={() => onView(resume.id)} variant="outline">
                <EyeIcon className="h-4 w-4" />
                View
              </Button>
              <Button onClick={() => onEdit(resume.id)} variant="outline">
                <PencilSquareIcon className="h-4 w-4" />
                Edit
              </Button>
              <Button onClick={() => onDelete(resume)} variant="ghost">
...
function MetaTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
      <p className="text-xs uppercase tracking-[0.22em] text-slate-600 dark:text-slate-400">{label}</p>
