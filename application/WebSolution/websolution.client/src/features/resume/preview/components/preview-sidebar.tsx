...
function ContactList({ resume }: { resume: ResumeDetailResponse }) {
  const items = [
    resume.personalInformation?.email
      ? { icon: <LinkIcon className="h-4 w-4" />, label: resume.personalInformation.email }
      : null,
    resume.personalInformation?.phoneNumber
      ? { icon: <PhoneIcon className="h-4 w-4" />, label: resume.personalInformation.phoneNumber }
      : null,
  ...
  if (items.length === 0) {
    return <p className="text-sm text-slate-600 dark:text-slate-400">No contact details available.</p>;
  }
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => (
        ...
        >
          <div className="mt-0.5 text-teal-700 dark:text-teal-300">{item.icon}</div>
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {resume.skills.length ? (
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">No skills added yet.</p>
      )}
    </div>
  );
}
...
function SocialLinks({ resume }: { resume: ResumeDetailResponse }) {
  if (!resume.personalInformation?.socialMediaLinks.length) {
    return <p className="text-sm text-slate-600 dark:text-slate-400">No social links available.</p>;
  }
    ...
    <div className="space-y-3">
      {resume.personalInformation.socialMediaLinks.map((link) => (
        ...
        >
          ...
          <span className="text-slate-600 dark:text-slate-400">{link.displayName ?? 'Open'}</span>
        </a>
      ))}
    </div>
  );
