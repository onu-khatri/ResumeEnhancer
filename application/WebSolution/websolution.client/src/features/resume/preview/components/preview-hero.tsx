            ...
            <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    ...
                    {contactDetails.length ? (
                        <ContactChips items={contactDetails} />
                    ) : null}
                </div>
    ...
    return (
        <div className="max-w-sm rounded-[1.75rem] border border-slate-300/70 bg-white/88 p-5 shadow-lg backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/75">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <p className="text-xs tracking-[0.24em] text-slate-600 uppercase dark:text-slate-400">
                        Last updated
                    </p>
                    <p className="text-sm font-medium text-slate-900 dark:text-white">
                        {formatDate(
                            resume.app_UpdateDate ?? resume.app_CreateDate,
                            {
                                day: 'numeric',
                                month: 'long',
                                year: 'numeric',
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300">
                This layout is tuned for fast scanning: clear hierarchy, strong
                spacing, and compact evidence blocks instead of dense walls of
                text.
            </p>
        </div>
    );
